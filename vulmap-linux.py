...
def getProductList():
	...
	# for RedHat Serial
	dpkg = "rpm -qai|grep -E '^(Version[[:space:]]+:|Name[[:space:]]+:)'|sed -e 's/[[:space:]]\{1,\}/ /g'|cut -d' ' -f3"
	...
	if pV == 2:
		...
		tempList = str(results).split('\\n')
	if (len(tempList)-1)%2 != 0:
		print("Error")
		sys.exit(0)
	for i in range(0,len(tempList)-1,2):
		tmpList = []
		tmpList.append(tempList[i])
		tmpList.append(tempList[i+1])
		tmpList.append("Centos")
